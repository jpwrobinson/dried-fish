...
read.csv('data/clean/dried_nutrient_estimates_long.csv') %>% 
    ...
    mutate(
           ...
           unit = ifelse(nutrient %in% c('cadmium', 'lead', 'mercury'), 'kg-1', unit),
           value = ifelse(nutrient %in% c('cadmium', 'lead', 'mercury'), value / 10, value),
           value = ifelse(nutrient %in% c('epa_dha', 'epa', 'dha'), value * 1000, value),
